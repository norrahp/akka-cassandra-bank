...
object Validation {
  ...
  // minimum value
  trait Minimum[A] extends ((A, Double) => Boolean) // numerical fields
  trait MinimumAbs[A] extends ((A, Double) => Boolean) // numerical fields
  ...
  def required[A](value: A)(implicit req: Required[A]) = req(value)
  def minimum[A](value: A, threshold: Double)(implicit min: Minimum[A]) = min(value, threshold)
  def minimumAbs[A](value: A, threshold: Double)(implicit min: MinimumAbs[A]) = min(value, threshold)
