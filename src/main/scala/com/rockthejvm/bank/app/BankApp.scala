...
object BankApp {
  ...
  def startHttpServer(bank: ActorRef[Command])(implicit system: ActorSystem[_]): Unit = {
    ...
    httpBindingFuture.onComplete {
      case Success(binding) =>
        ...
        system.log.info(s"System online at http://${address.getHostString}:${address.getPort}")
      case Failure(ex) =>
        system.log.error(s"Failed to bind server, because: $ex")
        system.terminate()
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    implicit val system = ActorSystem[RootCommand](rootBehavior, "BankSystem")
